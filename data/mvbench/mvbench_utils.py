### copied from https://github.com/EvolvingLMMs-Lab/lmms-eval/blob/80391ce3bfb5a19b32e7a19a2d9399e1378ed2dd/lmms_eval/tasks/mvbench/utils.py
import datetime
import json
import os
import re
...
# base_cache_dir = os.path.expanduser(base_cache_dir_)  
...
def mvbench_doc_to_visual(doc, cache_dir, sub_task):
    ...
    # elif os.path.basename(dataset_folder) in ["clevrer", "star"]:
    elif dataset_folder.split("/")[0] in ["clevrer", "star"]:
        ...
        if os.path.exists(alternative_video_path):
            ...
            eval_logger.error(f"Video path: {video_path} does not exist in data0613, please check.")
    else:
        eval_logger.error(f"Video path: {video_path} does not exist, please check.")
    return [video_path]


def mvbench_frames_doc_to_visual(doc, cache_dir, sub_task):
    # cache_dir = os.path.join(base_cache_dir, cache_name)
    # cache_dir = base_cache_dir
    cache_dir = os.path.expanduser(cache_dir)  
    dataset_folder = DATA_LIST[sub_task]
