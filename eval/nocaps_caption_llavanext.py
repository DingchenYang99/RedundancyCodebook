...
def eval_model(args):
    ...
    all_nocaps_data = []
    for domain in args.nocaps_split:
        print(f"loading {domain} data")
        all_nocaps_data += nocaps_data[domain]
    
    # all_nocaps_data = all_nocaps_data[:10]  # TODO debug
    ...
    for line in tqdm(all_nocaps_data):  # bs=1 only
        idx = line['image_id']
        ...
        # qs = "Provide a description of the image in two to three sentences."
        # qs = "Please describe this image. Write 2 to 3 sentences."
        # qs = "Please describe this image in detail. "  # dense caption # TODO
        # qs = "Please provide a detailed description of the given image, offering accurate and specific descriptions of the key information and details within the image. Note that you should describe the content of the image faithfully and objectively, without adding subjective inferences or commentary."
        ...
        # qs = "Please carefully observe the image and provide a caption for the image"
        # qs = "Please carefully observe the image and come up with a caption for the image" # copied from lmms_eval/tasks/nocaps/utils.py
        cur_prompt = qs
        
        image_path = os.path.join(args.nocaps_img_path, image_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nocaps_path", type=str, default="")
    parser.add_argument("--nocaps_split", type=list, default=[])
    ...
    args.nocaps_path = '/DATA3/yangdingchen/nocaps/'
    args.nocaps_img_path = args.nocaps_path + 'images/'
    args.nocaps_split = ["in-domain", "near-domain", "out-domain"]
    save_path = '/DATA2/yangdingchen/nocaps/'
    # args.nocaps_split = ["out-domain"]
    args.result_path = save_path + 'results/' + get_timestamp() 
    ...
    answer_file_prefix = 'llavaNext_nocaps_zeroshot_captions_image'
