...
from data.mantis_eval.mantis_eval_utils import load_data_for_mantis_eval
import warnings
...
def eval_model(args):
    ...
    device = "cuda"
    device_map = "auto"
    mantis_data = load_data_for_mantis_eval(args.mantis_path, args.mantis_ques_temp_path)
    ...
    model.eval()
    all_data = []
    for domain in args.mantis_split:
        print(f"loading {domain} data")
        all_data += mantis_data[domain]
    # all_data = all_data[:1]  # TODO for deb74
    # ug
    ...
    for line in tqdm(all_data):  # bs=1 only
        ...
        # torch.Size([num_imgs, 5, 3, 384, 384]) for anyres_max_4
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mantis_path", type=str, default="")
    parser.add_argument("--mantis_ques_temp_path", type=str, default="")
    parser.add_argument("--mantis_split", type=list, default=[])
    ...
    args.mantis_path = '/DATA2/yangdingchen/Mantis-Eval/mantis_eval/'
    args.mantis_ques_temp_path = Path('/home/lufan/Projects/PensieveV2/Mantis/mantis/benchmark/templates')
    args.mantis_split = ["test"]
    args.result_path = args.mantis_path + 'results/' + get_timestamp() 
    ...
    answer_file_prefix = 'llavaOV_mantis_test_zeroshot_multichoice_image'
