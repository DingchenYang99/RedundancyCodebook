...
from data.realWorldQA.realWorldQA_utils import load_data_for_realWorldQA_test
import warnings
...
def eval_model(args):
    ...
    rwqa_data = load_data_for_realWorldQA_test(args.rwqa_path)
    ...
    all_rwqa_data = []
    all_rwqa_data += rwqa_data['test']
    # all_rwqa_data = all_rwqa_data[:2]  # TODO debug
    ...
    for line in tqdm(all_rwqa_data):  # bs=1 only
        ...
        with torch.inference_mode():
            cont = model.generate(
                ...
                max_new_tokens=16,  # copy from lmms_eval/tasks/realworldqa/realworldqa.yaml
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rwqa_path", type=str, default="")
    # parser.add_argument("--rwqa_split", type=list, default=[])
    ...
    args.rwqa_path = '/DATA2/yangdingchen/RealWorldQA/'
    args.result_path = args.rwqa_path + 'results/' + get_timestamp() 
    ...
    answer_file_prefix = 'llavaNext_realworldQA_test_zeroshot_vqa_image'
