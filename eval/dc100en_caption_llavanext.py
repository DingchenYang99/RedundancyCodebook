...
from data.dc100en.dc100en_utils import *
import warnings
...
def eval_model(args):
    ...
    test_data = load_data_for_dc100en_test(args.data_path)['test']
    test_data = test_data[29:30]  # TODO debug
    answers_file = os.path.expanduser(os.path.join(args.result_path, args.answers_file_name))
    ans_file = open(answers_file, "w")
    print(f"save to {answers_file}")

    for line in tqdm(test_data):  # bs=1 only
        idx = line['image_id']
        ...
        with torch.inference_mode():
            cont = model.generate(
                ...
                max_new_tokens=512,  # TODO 64 is copied from lmms_eval/tasks/flickr30k/flickr30k_test_lite.yaml, 256 is the same as nocaps
                )
...
if __name__ == "__main__":
    ...
    args.data_path = '/DATA2/yangdingchen/DC100_EN/'
    args.result_path = args.data_path + 'results/' + get_timestamp() 
    ...
    answer_file_prefix = 'llavaNext_dc100en_test_zeroshot_captions_image'
